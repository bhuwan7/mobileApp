...
window.addEventListener("DOMContentLoaded",
    function(){
        ...
        setMessage("pen-turn");
        //making a class that checks the possibility of  clicking the square.
...
squaresArray.forEach(function (square){
    square.addEventListener('click', () => {
        ...
        //If not game over bear turn auto
        if (gameOverFlg === "0") {
            const squaresBox = document.getElementById("squaresBox");
            squaresBox.classList.add("js-unclickable"); //making sqaures box unclickable
            setTimeout(
                function() {
                    bearTurn();
                },
...
function isSelect(selectSquare){
    ...
    if(flag==="pen-flag"){
        ...
        selectSquare.classList.remove("js-clickable"); //removing the class that deteermines the possibility of square clickable
        //penguins win
...
function bearTurn() {
    ...
    while(bearTurnEnd === "0") {
        //searches for the attackable place for bear (attack)
        bearTurnEnd = isReach("bear");
        if (bearTurnEnd === "1") {
            gameOverFlg = "1";
            break; // end of while
        }
        //searches for the attackable place for penguin (defense)
        ...
        // select clickable bearturn randomly
...
// search for the reachable
function isReach(status) {
    let bearTurnEnd = "0"; // bear turn

    lineArray.some(function (line){
        let bearCheckCnt = 0; // checked number by bear
        let penCheckCnt = 0; // checked number by penguin

        line.forEach(function(square) {
            if(square.classList.contains("js-bear-checked")) {
                bearCheckCnt++; //number checked by bear
            }
            if(square.classList.contains("js-pen-checked")) {
                penCheckCnt++; //number checked by penguins
            }
        });
        if (status === "bear" && bearCheckCnt === 2 && penCheckCnt === 0) {
            bearTurnEnd = "1"; //bear reach line available
        }
        if (status === "penguins" && bearCheckCnt === 0 && penCheckCnt === 2) {
            bearTurnEnd = "1"; //penguins reach line available
        }

        // if bear is in reach point of penguins then search for the empty point
        if(bearTurnEnd === "1") {
